...
iris_especies <- data.frame(petal_length = data_sub$Petal.Length,
                            species = data_sub$Species)

table(data_sub$Species)
head(data_sub)

tapply(data_sub$Petal.Length,iris_especies)
...
df_virg <-subset(iris_especies,species == "virginica")
df_vers <-subset(iris_especies, species !="virginica")
...
d_cal <- cohens_efecto(df_vers$petal_length, df_virg$petal_length)
d_cal
